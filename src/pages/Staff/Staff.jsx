...
import { DeleteStaffDetails, DisplayStaffList, resetStaffStatus } from '../../store/staffSlice';
import { STATUS } from '../../store/Status';
import Sidebar from '../component/Sidebar';

function Staff() {
    const dispatch = useDispatch()
    const {StaffList, AlertData, Status} = useSelector(state => state.staffData)
    
    useEffect(() => {
        // Dispatch Redux action to fetch staff list
        dispatch(DisplayStaffList())
    },[dispatch])
    ...
    const deleteStaffHandler = (id) => {
        ...
        dispatch(DeleteStaffDetails(id))
    }
                        ...
                        <table class="min-w-full class='bg-red-300' leading-normal">
                            <thead >
                                <tr >
                                    ...
                                    <th class="px-5 bg-red-300 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-md font-semibold text-gray-600 uppercase tracking-wider">Created at</th>
                            ...
                            <tbody>
                            {
                                StaffList && StaffList.map((staff, index)=>(
                                    <tr>
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-md">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 w-10 h-10">
                                                    <img class="w-full h-full rounded-full" src={staff?.profilePicture} alt={staff?.name} />
                                                </div>
                                                <div class="ml-3">
                                                    <p class="text-gray-900 whitespace-no-wrap"> {staff?.name} - {staff?.email} </p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-md">
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-md">
                                            <p class="text-gray-900 whitespace-no-wrap"> {staff?.DOB}</p>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-md">
                                        ...
                                        {staff?.status == 1 ? <>
                                            ...
                                            <td class="px-5 py-5 border-b border-gray-200 bg-white  text-md">
                                                <p class="text-red-500   whitespace-no-wrap"> Deactive</p>
                                            </td>
                                        </>
                                        }
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-md">
                                            ...
                                            <span onClick={()=>deleteStaffHandler(staff.id)} class="relative cursor-pointer mt-3 inline-block px-3 py-1 text-md font-semibold text-green-900 leading-tight">
