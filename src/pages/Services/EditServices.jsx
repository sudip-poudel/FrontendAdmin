...
import { addService, getSingleServiceDetails, resetServiceStatus, updateService } from '../../store/serviceSlice';
...
function EditService() {
    ...
    const {singleServiceDetails, status, alertData} = useSelector((state)=>state.serviceData)
        ...
        <div class="bg-gradient-to-r from-red-200 to-green-200 p-8 rounded-md w-full">
            <div class=" flex items-center justify-between pb-3">
                <div className='flex'>
                    ...
                    <h2 class="text-blue-800 text-xl font-semibold uppercase">Edit Service</h2>
                </div>
            </div>
            <div className=' '>
                        ...
                        <form >
                            <div class="grid gap-y-3 grid-cols-6 gap-6">
                                ...
                                <div class="w-full col-span-6">
                                    ...
                                    <textarea value={Description} className='bg-white w-full h-60 p-3' onChange={(e)=>setDescription(e.target.value)} placeholder='Somthing About Service' name="Description" id="Description" ></textarea>
                                </div>
                            </div>
                        </form>
                    </div>
                    <div class="p-6 border-t border-gray-200 rounded-b">
                        <button onClick={handleUpdateBtn} class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-xl px-5 py-2.5 text-center" type="submit">Update Service</button>
                    </div>
                </div>
            </div>
        </div>
